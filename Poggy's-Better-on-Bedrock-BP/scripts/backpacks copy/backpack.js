...
function saveStructure1() {
	...
	for (let player of players) {
		let a = system.runInterval(() => {
			...
			player.sendMessage(
				"§cYou can't Open the Backpack while sprinting!",
			);
...
export function backPack() {
	...
	for (let player of players) {
		...
		if (
			items?.typeId == "better_on_bedrock:backpack" &&
			!player.hasTag("backpack2")
		) {
			player.addTag("backpack2");
			let Dimensions = world.getDimension(player.dimension.id);
			let entitee = Dimensions.spawnEntity(
