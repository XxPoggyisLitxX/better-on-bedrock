import { ItemStack, Location, world } from "@minecraft/server";
    ...
    for (let player of players) {
        let e = world.afterEvents.tick.subscribe((data) => {
            player.runCommandAsync(
                "structure save " + player.Id +
                    " ~-1 ~8 ~-1 ~1 ~3 ~1 true disk false",
            );
            player.runCommandAsync(
...
world.afterEvents.tick.subscribe((data) => {
    ...
    for (let player of players) {
        ...
        if (
            item?.typeId === "better_on_bedrock:backpack_uncommon" &&
            player.getComponent("movement").current ===
                player.getComponent("movement").value
        ) {
            ...
            !item?.typeId == "better_on_bedrock:backpack_uncommon" ||
            (items?.typeId == "better_on_bedrock:backpack_uncommon" &&
                player.getComponent("movement").current >
                    player.getComponent("movement").value)
        ) {
