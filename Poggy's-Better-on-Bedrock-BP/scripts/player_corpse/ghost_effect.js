...
export function stdout() {
    ...
    return world.getDimension("overworld").runCommandAsync(
        "effect @p slowness 0 0 true",
    );
}
...
world.afterEvents.itemUse.subscribe((data) => {
    ...
    if (
        items?.typeId == "better_on_bedrock:fixed_ghost_necklace" &&
        player.getItemCooldown("ghost") == 0
    ) {
        player.runCommandAsync("effect @p night_vision 10 10 true");
        player.startItemCooldown("ghost", 1200);
        items.getComponent("durability").damage =
            items.getComponent("durability").damage + 1;
