...
export const onPlayerDeath = () => {
    for (const player of world.getAllPlayers()) {
        ...
        if (
            ...
            player.addTag("death");
            let entity = player.dimension.spawnEntity(
                "better_on_bedrock:player_corpse",
                new Vector(
                    player.location.x,
                    player.location.y,
                    player.location.z,
...
world.afterEvents.itemReleaseCharge.subscribe(
    (data) => {
        for (const player of world.getAllPlayers()) {
            const item = player.getComponent("inventory").container.getItem(
                player.selectedSlot,
            );
            let newItem;
            switch (item?.typeId) {
                ...
                case "better_on_bedrock:iron_spear":
                    newItem = new ItemStack(
                        ItemTypes?.get("better_on_bedrock:iron_spear"),
                    );
                    player.removeTag("iron_spear");
                    break;
                case "better_on_bedrock:golden_spear":
                    newItem = new ItemStack(
                        ItemTypes?.get("better_on_bedrock:golden_spear"),
                    );
                    player.removeTag("golden_spear");
                    break;
                case "better_on_bedrock:diamond_spear":
                    newItem = new ItemStack(
                        ItemTypes?.get("better_on_bedrock:diamond_spear"),
                    );
            ...
            let e = system.runInterval(
                () => {
                    if (
                        ...
                        itemed.getComponent("durability").damage = item
                            .getComponent(
                                "durability",
                            ).damage += 1;
