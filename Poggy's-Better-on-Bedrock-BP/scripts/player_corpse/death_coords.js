...
export function onPlayerDeath(player) {
    ...
    if (playerHealt == 0 && isDeath == false) {
        ...
        player.runCommand(
            `tp @e[type=item,r=10] @e[type=better_on_bedrock:player_corpse, r=1`,
        );
...
function bounty_tier_page(damagingEntity, hitEntity) {
    ...
    form.title(`Dust?`);
    form.body(
        `Dusting the corpse will delete all items inside it. To recover your items, open the corpse's inventory.`,
    );
    ///buttons
    form.button(`Dust It!`);
