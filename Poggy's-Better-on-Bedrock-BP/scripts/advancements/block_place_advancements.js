...
world.afterEvents.blockPlace.subscribe((data) => {
	...
	for (let player of players) {
		if (
			...
			player.sendMessage(
				"[§2" + player.nameTag +
					"§f] Made The Advancement: §aMiner's Dream!",
			);
		...
		if (
			...
			player.sendMessage(
				"[§2" + player.nameTag +
					"§f] Made The Advancement: §aA Seedy Place!",
			);
		...
		if (
			...
			player.sendMessage(
				"[§2" + player.nameTag +
					"§f] Made The Advancement: §a§aBenchmarking!",
			);
	...
	({ block, player, brokenBlockPermutation, r }) => {
		...
		for (let slot = 0; slot < inventory.container.size; slot++) {
			...
			if (
				...
				!player.hasTag("stone_age")
			) {
				player.sendMessage(
					"[§2" + player.nameTag +
						"§f] Made The Advancement: §eStone Age!",
				);
				player.runCommandAsync("playsound normal_quest @p");
			...
			if (
				...
				!player.hasTag("stardust_ore")
			) {
				player.sendMessage(
					"[§2" + player.nameTag +
						"§f] Made The Advancement: §5Magic Stones!",
				);
