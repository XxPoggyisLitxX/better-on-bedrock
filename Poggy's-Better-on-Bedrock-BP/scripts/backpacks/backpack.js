import { EquipmentSlot, system, Vector, world } from "@minecraft/server";
export const backpack = (player, backpackId, size = 0) => {
	try {
		const backpacks = player.dimension.getEntities({
			tags: ["backpack:" + backpackId],
		});
		if (backpacks.length > 2) {
			backpacks.forEach((b) => b.triggerEvent("despawn_backpack"));
		}
		...
		if (
			...
			const [entity] = player.dimension.getEntities({
				tags: ["backpack:" + player.lastBackpackId],
			});
			if (entity) {
				entity.runCommand(
					`structure save "${player.lastBackpackId}" ~-5 ~8 ~-5 ~+5 ~+5 ~+5 true disk false`,
				);
			...
			return (
				player.dimension.getEntities({
						...
						entity.getComponent("minecraft:rideable")?.getRiders()
							.find((e) =>
								(e instanceof Player) && e.name == player.name
							)
					)
					? true
					: false
		...
		if (
			...
			if (entity) {
				entity.runCommand(
					`structure save "${backpackId}" ~-5 ~8 ~-5 ~+5 ~+5 ~+5 true disk false`,
				);
				entity.triggerEvent("despawn_backpack");
			}
		...
		const [newEntity] = player.dimension.getEntities({
			tags: ["backpack:" + backpackId],
		});
		if (!newEntity) {
			...
			if (
				...
				const e = player.dimension.spawnEntity(
					size == 0 ? "better_on_bedrock:backpack" : (
						size == 1
		...
		world.getAllPlayers().filter(
			(player) => {
				...
				if (
					...
					backpack(
						player,
						JSON.parse(handItem.getLore()[0]).id,
						JSON.parse(handItem.getLore()[0])?.size,
					);
				} else if (
					handItem?.typeId == "better_on_bedrock:backpack" ||
					...
					if (entity) {
						...
						entity.runCommand(
							`structure save "${player.lastBackpackId}" ~-5 ~6 ~-5 ~+5 ~+5 ~+5 true disk false`,
						);
						entity.triggerEvent("despawn_backpack");
					}
...
const generateSnowflake = () =>
	((Date.now() - 1420070400000) * Math.pow(2, 22)).toString();
