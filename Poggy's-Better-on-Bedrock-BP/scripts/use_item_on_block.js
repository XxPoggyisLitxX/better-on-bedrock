...
world.beforeEvents.itemUseOn.subscribe((use) => {
	...
	if (
		...
		console.warn("test");
		world.getDimension(player.dimension.id).runCommandAsync(
			"give @p better_on_bedrock:broken_open_coconut 1 0",
		);
		world.getDimension(player.dimension.id).runCommandAsync(
			"clear @p better_on_bedrock:coconut_nut 0 1",
		);
	}
	...
	if (
		...
		(blockTest?.typeId == "minecraft:grass" ||
			blockTest?.typeId == "minecraft:dirt" ||
			blockTest?.typeId == "minecraft:coarse_dirt")
	) {
		console.warn("test");
		world.getDimension(player.dimension.id).runCommandAsync(
			"playsound use.gravel @p",
		);
	}
	if (
