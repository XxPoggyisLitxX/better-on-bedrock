...
const bounty_tier_page = (player) => {
	...
	if (tiersCompleted >= 3) {
		form.button(
			"§fBeyond the Overworld\n[§aUnlocked§f]",
			"textures/items/netherite_ingot",
		);
	} else {
		form.button(
			"§7Beyond the Overworld\n[§cLocked§7]",
			"textures/items/netherite_ingot",
		);
	}
				...
				case 0:
					if (player.getDynamicProperty("tiersCompleted") >= 0) {
						time_to_mine(player);
							...
							'§cYou need to complete "Unknown" in order to unlock this tier.',
						);}
					break;
				case 1:
					if (player.getDynamicProperty("tiersCompleted") >= 1) {
						adventure_delight(player);
							...
							'§cYou need to complete "Time to Mine" in order to unlock this tier.',
						);}
					break;
				case 2:
					if (player.getDynamicProperty("tiersCompleted") >= 2) {
						monster_looter(player);
							...
							'§cYou need to complete "Adventure Delight" in order to unlock this tier.',
						);}
					break;
				case 3:
					if (player.getDynamicProperty("tiersCompleted") >= 3) {
						beyond_the_overworld(player);
							...
							'§cYou need to complete "Monster Looter" in order to unlock this tier.',
						);}
					break;
				case 4:
					if (player.getDynamicProperty("tiersCompleted") >= 4) {
						the_willager(player);
							...
							'§cYou need to complete "Beyond The Overworld" in order to unlock this tier.',
						);}
					break;
				case 5:
					if (player.getDynamicProperty("tiersCompleted") >= 5) {
						more_food(player);
							...
							'§cYou need to complete "The Willager" in order to unlock this tier.',
						);}
					break;
			}
		},
...
world.afterEvents.itemUse.subscribe((data) => {
	...
	if (items?.typeId == "better_on_bedrock:quest_paper") {
		player.runCommandAsync(
			"replaceitem entity @s slot.weapon.mainhand 0 better_on_bedrock:bounty_paper_open",
		);
		if (!player.getDynamicProperty("quests")) {
			player.setDynamicProperty(
				"quests",
