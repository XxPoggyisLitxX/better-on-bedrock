...
const quests = [
					...
					switch (response?.selection) {
						case 0:
							...
							player.onScreenDisplay.setTitle(
								"sadEndermanQuestStart",
							);
							player.sendMessage("§aQuest Started!");
							savedQuests.find((q) => q.id == Quests.sadEnderman)
								.s = QuestStatus.Busy;
	...
	},
	{
		...
		locked: (player) =>
			player.sendMessage(
				'§cYou need to complete "Sad Enderman" in order to unlock this quest.',
			),
		info: (player) => {
			const form = new ActionFormData();
				...
				(response) => {
					switch (response?.selection) {
						case 0:
							...
							player.onScreenDisplay.setTitle(
								"bountyTimeQuestStart",
							);
							player.sendMessage("§aQuest Started!");
							savedQuests.find((q) => q.id == Quests.bountyTime)
								.s = QuestStatus.Busy;
							player.setDynamicProperty(
								"quests2",
								JSON.stringify(savedQuests),
	...
	},
	{
		...
		locked: (player) =>
			player.sendMessage(
				'§cYou need to complete "Bounty Time" in order to unlock this quest.',
			),
		info: (player) => {
			const form = new ActionFormData();
				...
				(response) => {
					switch (response?.selection) {
						case 0:
							...
							player.onScreenDisplay.setTitle(
								"logCollectorQuestStart",
							);
							player.sendMessage("§aQuest Started!");
							savedQuests.find((q) => q.id == Quests.logCollector)
								.s = QuestStatus.Busy;
							player.setDynamicProperty(
								"quests2",
								JSON.stringify(savedQuests),
			...
			form.show(player).then(
				(response) => {
					switch (response?.selection) {
						case 0:
							const quest = savedQuests.find((q) =>
								q.id == Quests.logCollector
							);
							if (quest.s != QuestStatus.Claimed) {
								player.runCommandAsync("xp 250");
								player.runCommandAsync(
									"give @s better_on_bedrock:disc_travelers 1",
								);
								quest.s = QuestStatus.Claimed;
								player.setDynamicProperty(
									"quests2",
	...
	},
	{
		...
		locked: (player) =>
			player.sendMessage(
				'§cYou need to complete "Log Collector" in order to unlock this quest.',
			),
		info: (player) => {
			const form = new ActionFormData();
			form.title("§fOre Collector");
			form.body(
				"Get 1 Coal Ore Block.\nRewards: Efficiency 5 Iron Pickaxe and 250 XP",
			);
			form.button("Ok");
			form.show(player);
		},
				...
				(response) => {
					switch (response?.selection) {
						case 0:
							...
							player.onScreenDisplay.setTitle(
								"oreCollectorQuestStart",
							);
							player.sendMessage("§aQuest Started!");
							savedQuests.find((q) => q.id == Quests.oreCollector)
								.s = QuestStatus.Busy;
							player.setDynamicProperty(
								"quests2",
								JSON.stringify(savedQuests),
					...
					switch (response?.selection) {
						case 0:
							...
							if (quest.s != QuestStatus.Claimed) {
								...
								player.runCommandAsync(
									"give @s better_on_bedrock:iron_pickaxe 1",
								);
								quest.s = QuestStatus.Claimed;
								player.setDynamicProperty(
									"quests2",
	...
	},
	{
		...
		locked: (player) =>
			player.sendMessage(
				'§cYou need to complete "Ore Collector" in order to unlock this quest.',
			),
		info: (player) => {
			const form = new ActionFormData();
				...
				(response) => {
					switch (response?.selection) {
						case 0:
							...
							player.sendMessage("§aQuest Started!");
							savedQuests.find((q) => q.id == Quests.evokerSpells)
								.s = QuestStatus.Busy;
							player.setDynamicProperty(
								"quests2",
								JSON.stringify(savedQuests),
