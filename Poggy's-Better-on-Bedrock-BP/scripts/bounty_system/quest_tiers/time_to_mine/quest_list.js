...
const quests = [
				...
				(response) => {
					switch (response?.selection) {
						case 0:
							...
							savedQuests.find((q) => q.id == Quests.Metallis).s =
								QuestStatus.Busy;
	...
	},
	{
		...
		locked: (player) =>
			player.sendMessage(
				'§cYou need to complete "Metallis" in order to unlock this quest.',
			),
		info: (player) => {
			const form = new ActionFormData();
				...
				(response) => {
					switch (response?.selection) {
						case 0:
							...
							player.sendMessage("§aQuest Started!");
							savedQuests.find((q) => q.id == Quests.LightMyDay)
								.s = QuestStatus.Busy;
							player.setDynamicProperty(
								"quests",
								JSON.stringify(savedQuests),
	...
	},
	{
		...
		locked: (player) =>
			player.sendMessage(
				'§cYou need to complete "Light My Day" in order to unlock this quest.',
			),
		info: (player) => {
			const form = new ActionFormData();
	...
	},
	{
		...
		locked: (player) =>
			player.sendMessage(
				'§cYou need to complete "Witchcraft Blue" in order to unlock this quest.',
			),
		info: (player) => {
			const form = new ActionFormData();
			form.title("§fAmethysts");
			form.body(
				"Get 32 Amethyst Shards.\n§7Rewards: 4 Blocks of Amethyst and 50 XP",
			);
			form.button("Ok");
			form.show(player);
		},
				...
				(response) => {
					switch (response?.selection) {
						case 0:
							...
							player.sendMessage("§aQuest Started!");
							savedQuests.find((q) => q.id == Quests.Amethysts)
								.s = QuestStatus.Busy;
							player.setDynamicProperty(
								"quests",
								JSON.stringify(savedQuests),
			...
			form.show(player).then(
				(response) => {
					switch (response?.selection) {
						case 0:
							const quest = savedQuests.find((q) =>
								q.id == Quests.Amethysts
							);
							if (quest.s != QuestStatus.Claimed) {
								player.runCommandAsync("xp 50");
								player.runCommandAsync(
									"give @s amethyst_block 4",
								);
								quest.s = QuestStatus.Claimed;
								player.setDynamicProperty(
									"quests",
	...
	},
	{
		...
		locked: (player) =>
			player.sendMessage(
				'§cYou need to complete "Amethysts" in order to unlock this quest.',
			),
		info: (player) => {
			const form = new ActionFormData();
				...
				(response) => {
					switch (response?.selection) {
						case 0:
							...
							player.sendMessage("§aQuest Started!");
							savedQuests.find((q) => q.id == Quests.Diamonds).s =
								QuestStatus.Busy;
							player.setDynamicProperty(
								"quests",
								JSON.stringify(savedQuests),
