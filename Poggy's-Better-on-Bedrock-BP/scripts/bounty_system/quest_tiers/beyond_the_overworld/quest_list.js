...
const quests = [
				...
				(response) => {
					switch (response?.selection) {
						case 0:
							...
							savedQuests.find((q) => q.id == Quests.ThatsFine)
								.s = QuestStatus.Busy;
		...
		claim: (player) => {
			...
			form.show(player).then(
				(response) => {
					switch (response?.selection) {
						case 0:
							const quest = savedQuests.find((q) =>
								q.id == Quests.ThatsFine
							);
	...
	},
	{
		...
		locked: (player) =>
			player.sendMessage(
				'§cYou need to complete "That\'s fine" in order to unlock this quest.',
			),
		info: (player) => {
			const form = new ActionFormData();
				...
				(response) => {
					switch (response?.selection) {
						case 0:
							...
							player.sendMessage("§aQuest Started!");
							savedQuests.find((q) => q.id == Quests.Snowwhite)
								.s = QuestStatus.Busy;
							player.setDynamicProperty(
								"quests4",
								JSON.stringify(savedQuests),
	...
	},
	{
		...
		locked: (player) =>
			player.sendMessage(
				'§cYou need to complete "Snow white?" in order to unlock this quest.',
			),
		info: (player) => {
			const form = new ActionFormData();
				...
				(response) => {
					switch (response?.selection) {
						case 0:
							...
							player.sendMessage("§aQuest Started!");
							savedQuests.find((q) => q.id == Quests.Netherite)
								.s = QuestStatus.Busy;
							player.setDynamicProperty(
								"quests4",
								JSON.stringify(savedQuests),
	...
	},
	{
		...
		locked: (player) =>
			player.sendMessage(
				'§cYou need to complete "Nether ite?" in order to unlock this quest.',
			),
		info: (player) => {
			const form = new ActionFormData();
				...
				(response) => {
					switch (response?.selection) {
						case 0:
							player.removeTag("unlocked_2");
							player.onScreenDisplay.setTitle(
								"enderPlayerQuestStart",
							);
							player.sendMessage("§aQuest Started!");
							savedQuests.find((q) => q.id == Quests.EnderPlayer)
								.s = QuestStatus.Busy;
							player.setDynamicProperty(
								"quests4",
								JSON.stringify(savedQuests),
	...
	},
	{
		...
		locked: (player) =>
			player.sendMessage(
				'§cYou need to complete "Ender Player" in order to unlock this quest.',
			),
		info: (player) => {
			const form = new ActionFormData();
			form.title("§fNether Bed");
			form.body(
				"Get a Respawn Anchor.\n§7Rewards: 8 Glowstone and 75 XP",
			);
			form.button("Ok");
			form.show(player);
		},
				...
				(response) => {
					switch (response?.selection) {
						case 0:
							...
							player.sendMessage("§aQuest Started!");
							savedQuests.find((q) => q.id == Quests.NetherBed)
								.s = QuestStatus.Busy;
							player.setDynamicProperty(
								"quests4",
								JSON.stringify(savedQuests),
	...
	},
	{
		...
		locked: (player) =>
			player.sendMessage(
				'§cYou need to complete "Nether Bed" in order to unlock this quest.',
			),
		info: (player) => {
			const form = new ActionFormData();
				...
				(response) => {
					switch (response?.selection) {
						case 0:
							player.removeTag("unlocked_2");
							player.onScreenDisplay.setTitle(
								"movableChestQuestStart",
							);
							player.sendMessage("§aQuest Started!");
							savedQuests.find((q) => q.id == Quests.MovableChest)
								.s = QuestStatus.Busy;
							player.setDynamicProperty(
								"quests4",
								JSON.stringify(savedQuests),
	...
	},
	{
		...
		locked: (player) =>
			player.sendMessage(
				'§cYou need to complete "Movable Chest" in order to unlock this quest.',
			),
		info: (player) => {
			const form = new ActionFormData();
				...
				(response) => {
					switch (response?.selection) {
						case 0:
							...
							player.sendMessage("§aQuest Started!");
							savedQuests.find((q) => q.id == Quests.DragonEgg)
								.s = QuestStatus.Busy;
							player.setDynamicProperty(
								"quests4",
								JSON.stringify(savedQuests),
			...
			form.show(player).then(
				(response) => {
					switch (response?.selection) {
						case 0:
							const quest = savedQuests.find((q) =>
								q.id == Quests.DragonEgg
							);
							if (quest.s != QuestStatus.Claimed) {
								player.runCommandAsync("xp 175");
								player.runCommandAsync(
									"xp enchanted_golden_apple",
								);
								quest.s = QuestStatus.Claimed;
								player.setDynamicProperty(
									"quests4",
