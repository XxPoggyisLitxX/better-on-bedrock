...
const quests = [
                ...
                (response) => {
                    switch (response?.selection) {
                        case 0:
                            ...
                            savedQuests.find((q) => q.id == Quests.BowMaster)
                                .s = QuestStatus.Busy;
    ...
    },
    {
        ...
        locked: (player) =>
            player.sendMessage(
                '§cYou need to complete "Zombie Slayer" in order to unlock this quest.',
            ),
        info: (player) => {
            const form = new ActionFormData();
            form.title("§fStaying Healthy");
            form.body(
                "Get 16 Healthy Carrots.\n§7Rewards: 8 Healthy Carrots and 10 XP",
            );
            form.button("Ok");
            form.show(player);
        },
                ...
                (response) => {
                    switch (response?.selection) {
                        case 0:
                            ...
                            player.onScreenDisplay.setTitle(
                                "stayingHealthyQuestStart",
                            );
                            player.sendMessage("§aQuest Started!");
                            savedQuests.find((q) =>
                                q.id == Quests.StayingHealthy
                            ).s = QuestStatus.Busy;
                            player.setDynamicProperty(
                                "quests5",
                                JSON.stringify(savedQuests),
    ...
    },
    {
        ...
        locked: (player) =>
            player.sendMessage(
                '§cYou need to complete "Creeper Hunter" in order to unlock this quest.',
            ),
        info: (player) => {
            const form = new ActionFormData();
                ...
                (response) => {
                    switch (response?.selection) {
                        case 0:
                            ...
                            player.sendMessage("§aQuest Started!");
                            savedQuests.find((q) => q.id == Quests.ArmoredUp)
                                .s = QuestStatus.Busy;
                            player.setDynamicProperty(
                                "quests5",
                                JSON.stringify(savedQuests),
    ...
    },
    {
        ...
        locked: (player) =>
            player.sendMessage(
                '§cYou need to complete "Stringy Situation" in order to unlock this quest.',
            ),
        info: (player) => {
            const form = new ActionFormData();
            form.title("§fWillager Hat");
            form.body(
                "Get the Willager's Hat.\n§7Rewards: 1 Enchanted Golden Apple and 200 XP",
            );
            form.button("Ok");
            form.show(player);
        },
                ...
                (response) => {
                    switch (response?.selection) {
                        case 0:
                            player.removeTag("unlocked_2");
                            player.onScreenDisplay.setTitle(
                                "willagerHatQuestStart",
                            );
                            player.sendMessage("§aQuest Started!");
                            savedQuests.find((q) => q.id == Quests.WillagerHat)
                                .s = QuestStatus.Busy;
                            player.setDynamicProperty(
                                "quests5",
                                JSON.stringify(savedQuests),
