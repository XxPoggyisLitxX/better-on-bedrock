...
const stardustItems = {
    ...
    "better_on_bedrock:netherite_chestplate":
        "better_on_bedrock:stardust_chestplate",
    "better_on_bedrock:netherite_leggings":
        "better_on_bedrock:stardust_leggings",
                ...
                case 0:
                    if (getItemCount(player, "minecraft:gold_ingot") > 0) {
                        const newItem = new ItemStack(
                            ItemTypes?.get(gildedNetheriteItems[item.typeId]),
                        );
                        newItem.getComponent(ItemEnchantsComponent.componentId)
                            .enchantments = item.getComponent(
                                ItemEnchantsComponent.componentId,
                            )
                                .enchantments;
                        inventory.setItem(player.selectedSlot, newItem);
                        player.runCommandAsync(
                            "clear @p minecraft:gold_ingot 0 1",
                        );
                    } else {
                        player.sendMessage(
                            "§cYou don't have enough Gold Ingots",
                        );
                    }
                    break;
                case 1:
                    if (
                        getItemCount(
                            player,
                            "better_on_bedrock:stardust_ingot",
                        ) > 0
                    ) {
                        const newItem = new ItemStack(
                            ItemTypes?.get(stardustItems[item.typeId]),
                        );
                        newItem.getComponent(ItemEnchantsComponent.componentId)
                            .enchantments = item.getComponent(
                                ItemEnchantsComponent.componentId,
                        ...
                        inventory.setItem(player.selectedSlot, newItem);
                        player.runCommandAsync(
                            "clear @p better_on_bedrock:stardust_ingot 0 1",
                        );
                    } else {
                        player.sendMessage(
                            "§cYou don't have enough Stardust Ingots",
                        );
                    }
                    break;
            }
