    ...
    */
        ...
        if (lookingEntities.length != 0) {
            if (
                ...
                let addonID = itemComponent != null
                    ? itemComponent.itemStack.typeId.split(":")[0]
                    : lookingEntities[0].typeId.split(":")[0].replaceAll(
                        "_",
                        " ",
                    )
                        .replace(/(\b[a-z](?!\s))/g, (x) => {
                ...
                if (
                    lookingEntities[0].getComponent("item")?.typeId.includes(
                        "minecraft",
                    )
                ) {
                    player.runCommandAsync(
                        ...
                        }.name"}, {"text":" x${itemComponent.itemStack.amount}\n§9§o${addonID}"}]}`,
                    );
                }
            ...
            if (lookingBlock) {
                ...
                let addonID = lookingBlock.typeId.split(":")[0].replaceAll(
                    "_",
                    " ",
                )
                    .replace(/(\b[a-z](?!\s))/g, (x) => {
...
export function getHealthText(currentHealth, maxHealth) {
    ...
    {
        heartsToShow = "§7Health: " + Math.round(currentHealth) + " / " +
            maxHealth;
    }
