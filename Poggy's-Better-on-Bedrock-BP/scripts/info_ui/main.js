...
export const toolTip = (player) => {
    ...
    if (
        ...
        const tameProbability =
            entity.getComponent("minecraft:tameable")?.probability.toFixed(2) ||
            0;
        const isBaby = entity.getComponent("minecraft:is_baby") != null;
        const entityId = entity.typeId.split(":")[1];
        const addonIdentifier = item != null
            ? item.itemStack.typeId.split(":")[0]
            : entity.typeId.split(":")[0];

        const addonId = addonIdentifier
            .replaceAll("_", " ")
            .replace(/(\b[a-z](?!\s))/g, (x) => x.toUpperCase());

        if (item) {
            const isBlock = MinecraftBlockTypes.getAllBlockTypes().map((
                block,
            ) => block.id).includes(item.itemStack.typeId);
                    ...
                    rawtext: [
                        {
                            translate: entity.typeId == "minecraft:player"
                                ? "Player"
                                : "entity." + ((addonIdentifier == "minecraft"
                                    ? ""
                                    : entity.typeId.split(":")[0] + ":") +
                                    entityId) +
                                    ".name",
        ...
        const addonId = block.typeId.split(":")[0]
            .replaceAll("_", " ")
            .replace(/(\b[a-z](?!\s))/g, (x) => x.toUpperCase());
        ...
        const monsterEggStoneType = block.permutation.getState(
        ...
        const prismarineBlockType = block.permutation.getState(
        ...
        const coralHangTypeBit = block.permutation.getState(
            "coral_hang_type_bit",
        );
        ...
        if (crops[block.typeId]) {
            ...
            const currentGrowth =
                block.permutation.getState("growth") / maxGrowth ||
                block.permutation.getState("better_on_bedrock:growth_stage") /
                    maxGrowth;
            const percentage = Math.round(
                (currentGrowth !== 0 ? currentGrowth.toFixed(2) : 0) * 100,
            );
            growth = percentage == 100 ? "§aGrown" : `${percentage}%`;
        }
        ...
        const blockName = (
            ...
            ((saplingType && !block.typeId.includes("bamboo_sapling"))
                ? "." + (type[saplingType] || saplingType)
                : "") +
            (oldLeafType ? "." + (type[oldLeafType] || oldLeafType) : "") +
            (newLeafType ? "." + (type[newLeafType] || newLeafType) : "") +
            (woodType
                ? "." + (strippedBit ? "stripped." : "") +
                    (block.typeId.includes("slab") && woodType == "dark_oak"
                        ? type[woodType]
                        : woodType)
                : "") +
            (flowerType ? "." + (type[flowerType] || flowerType) : "") +
            (doublePlantType
                ? "." + (type[doublePlantType] || doublePlantType)
                : "") +
            (tallGrassType
                ? "." + (type[tallGrassType] || tallGrassType)
                : "") +
            (monsterEggStoneType
                ? "." +
                    (type.monsterEgg[monsterEggStoneType] ||
                        monsterEggStoneType)
                : "") +
            ...
            (sandStoneType
                ? "." + (type[sandStoneType] || sandStoneType)
                : "") +
            (prismarineBlockType
                ? "." +
                    (type.prismarine[prismarineBlockType] ||
                        prismarineBlockType)
                : "") +
            (chiselType ? "." + chiselType : "") +
            (damage ? "." + (type.damage[damage] || damage) : "") +
            (coralColor ? "." + coralColor + (deadBit ? "_dead" : "") : "") +
            (block.typeId.split(":")[1] == "coral_fan_hang"
                ? (deadBit ? "_dead" : "") + "." +
                    (coralHangTypeBit ? "pink_fan" : "blue_fan")
                : "") +
            (block.typeId.split(":")[1] == "coral_fan_hang2"
                ? (deadBit ? "_dead" : "") + "." +
                    (coralHangTypeBit ? "red_fan" : "purple_fan")
                : "") +
            (block.typeId.split(":")[1] == "coral_fan_hang3"
                ? (deadBit ? "_dead" : "") + "." +
                    (coralHangTypeBit ? "" : "yellow_fan")
                : "") +
            (stoneSlabType
                ? "." + (type.slab[stoneSlabType] || stoneSlabType)
                : "") +
            (stoneSlabType2
                ? "." + (type.slab[stoneSlabType2] || stoneSlabType2)
                : "") +
            (stoneSlabType3
                ? "." + (type.slab[stoneSlabType3] || stoneSlabType3)
                : "") +
            (stoneSlabType4
                ? "." + (type.slab[stoneSlabType4] || stoneSlabType4)
                : "");
...
export const healthToText = (currentHealth, maxHealth) => {
    ...
    if (maxHealth <= 40) {
        ...
        const hearts = Hearts.heart.repeat(health) +
            (half ? Hearts.halfHeart : "") + Hearts.emptyHeart.repeat(empty);
