...
world.afterEvents.itemUse.subscribe((use) => {
	...
	if (!player.hasTag("inUI")) {
		...
		if (
			...
			item?.typeId == "better_on_bedrock:waypoint_marker" &&
			blockTest?.typeId == "better_on_bedrock:waystone"
		) {
			UI(player);
			player.addTag("inUI");
		} else if (
			/*player.dimension.id == 'minecraft:overworld' && */ item?.typeId ==
					"better_on_bedrock:waypoint_marker" &&
				player.getItemCooldown("marker") == 0
		) {
			UI2(player);
		} else if (
			player.dimension.id == "minecraft:overworld" &&
			...
			player.runCommandAsync(
				`tellraw @s {"rawtext":[{"text":"§cYour Waystone Marker is on cooldown!"}]}`,
			);
		}
		if (
			player.dimension.id == "minecraft:overworld" &&
			...
			item?.typeId != "better_on_bedrock:waypoint_marker" &&
			blockTest?.typeId == "better_on_bedrock:waystone" &&
			warpTags.length > 0 && !player.isSneaking == true
		) {
			UI4(player);
			player.addTag("inUI");
		} else if (
...
world.afterEvents.blockBreak.subscribe(
	({ block, player, brokenBlockPermutation, r }) => {
		if (
			brokenBlockPermutation.type.id == "better_on_bedrock:waystone" &&
			player.getTags().filter((v) => v.startsWith("Warp:"))
		) {
			console.warn("1");
			const warpTags = player.getTags().filter((v) =>
				v.startsWith("Warp:")
			);
			console.warn("2");
			for (let tag of warpTags) {
				...
				console.warn("4");
				const test =
					`Warp:${warpName}-${block.location.x}|${block.location.y}|${block.location.z}|${player.dimension.id}`;
				console.warn("5");
...
function UI2(player, entity) {
	...
	const warpMenu = new ActionFormData()
		...
		.body(
			"To add another waystone, crouch and interact on the waystone.\n",
		);
	const warpTags = player.getTags().filter((v) => v.startsWith("Warp:"));
	for (let tag of warpTags) {
	...
	if (warpTags.length > 0) {
			...
			form.show(player).then((r) => {
				...
				if (r.selection == 0 && getPlayerExperienceLevel(player) >= 3) {
					if (selectedWarp.includes("nether")) {
						player.runCommandAsync(
							`tellraw @s {"rawtext":[{"text":"§anether!"}]}`,
						);
						let [, warpName, xCord, yCord, zCord, waystoneDim] =
							selectedWarp
								.match(
									/(?:Warp:)(.*?)-((?:-|)\d+)\|((?:-|)\d+)\|((?:-|)\d+)\|(minecraft:.+)/,
								);
						...
						player.teleport({
							...
							y: Number(yCord),
							z: Number(zCord),
						}, { dimension: world.getDimension(waystoneDim) });
						player.runCommandAsync(
							`tellraw @s {"rawtext":[{"text":"§aTeleported to Waystone §e${warpName}!"}]}`,
						);
						...
						r.selection == 0 && getPlayerExperienceLevel(player) < 3
					) {
					...
					player.runCommandAsync(
						`tellraw @s {"rawtext":[{"text":"§cYou don't have enough levels!"}]}`,
					);
				}
				if (r.selection == 1) {
					player.removeTag(selectedWarp);
					player.runCommandAsync(
						`tellraw @s {"rawtext":[{"text":"§aRemoved Waystone ${warpName}!"}]}`,
					);
		...
		player.runCommandAsync(
			`tellraw @s {"rawtext":[{"text":"§cYou have no Waystones set!"}]}`,
		);
...
function UI(player, r) {
	...
	new ModalFormData()
		...
		.show(player).then((r) => {
			...
			var test =
				`Warp:${warpName}-${getBlockLocation.x}|${getBlockLocation.y}|${getBlockLocation.z}|${player.dimension.id}`;
			...
			if (warpTags.find((v) => v.includes(warpName))) {
				...
				player.addTag(test);
				player.runCommandAsync(
					`tellraw @s {"rawtext":[{"text":"§aSet Waystone to: §e${warpName}!"}]}`,
				);
				player.runCommandAsync(`playsound note.pling @s ~~~ 1 1.5`);
			} else if (warpTags.length >= 25) {
				player.runCommandAsync(
					`tellraw @s {"rawtext":[{"text":"§cYou have the max amount of Waystones set already!"}]}`,
				);
				player.runCommandAsync(`playsound note.pling @s ~~~ 1 0.5`);
			} else {
				player.addTag(test);
...
function UI3(player, entity) {
	...
	addStone.textField(
		'§6Enter the Waystone name\n\n§7If this field is not filled, the Waystone name will be "Default".\n\nIf a name is the same as a current Waystone, the old Waystone will be overwritten.\n\n§7You have §c' +
			warpTags.length + "/25 Waystones set.",
		"   ",
	);
	...
	if (warpTags.length > 0) {
		...
		warpMenu.show(player).then((r) => {
			...
			new ActionFormData()
				...
				.body(
					"§c§lRemove this wasytone and save it to update this waystone to the new system.",
				)
				.button("§kunknown")
				...
				.show(player).then((r) => {
					...
					if (
						r.selection == 0 &&
						getPlayerExperienceLevel(player) >= 3
					) {
						if (selectedWarp.includes("nether")) {
							player.runCommandAsync(
								`tellraw @s {"rawtext":[{"text":"§anether!"}]}`,
